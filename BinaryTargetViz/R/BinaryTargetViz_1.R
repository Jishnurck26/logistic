    ...
    ui <- navbarPage(
      ...
      tabPanel(
        ...
        fluidPage(
          ...
          column(8, tableOutput("crosstab987"), tableOutput("count987"))
        )
      )
    )
    ...
    server <- function(input, output) {
...

      {
        ...
        output$text <- renderText({
          k <-
            c(
              "K2 Analytics Finishing School Pvt. Ltd.",
              "     Website: http://www.k2analytics.co.in"
            )
          k
        })
        ...
        crstbl2 <- reactive({
          ...
          tb2 <-
            as.data.frame.matrix(xtabs(kop$Ta ~ kop$range + kop$y))
          per <- tb2 * 100 / tb
        ...
        crstbl3 <- reactive({
          ...
          tb2 <-
            as.data.frame.matrix(xtabs(kop$Ta ~ kop$x + kop$range))
          per <- tb2 * 100 / tb
        ...
        crstbl4 <- reactive({
          ...
          tb <-
            as.data.frame.matrix(xtabs( ~ kop$range1 + kop$range))
          tb2 <-
      ...
      freq <- reactive({
        ...
        kob <-
          list(
            Var1 = "Total",
            Freq.x = nrow(kop),
            Freq.y = percent(nrow(kop) / nrow(kop))
          )
      ...
      qunt <- reactive({
        ...
        in4 <- as.numeric(in3)
        in4 <- sort(in4)
        QUANTILE <- apply(kop[c(1:ncol(kop))] ,
                          2 ,
                          quantile ,
                          probs = in4 / 100,
                          na.rm = TRUE)
